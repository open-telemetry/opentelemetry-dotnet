...
| `OTEL_SERVICE_VERSION`     | Sets the value of the `service.version` resource attribute. If `service.version` is also provided in `OTEL_RESOURCE_ATTRIBUTES`, then `OTEL_SERVICE_VERSION` takes precedence. |
