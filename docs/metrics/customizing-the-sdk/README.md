...
because it is not evaluated until the process is running and a measurement is
recorded. The OpenTelemetry SDK will NOT throw exceptions when running. Any
mistakes made using this overload will cause the `View` to be ignored.
Exceptions will be written to the internal logging only. Use this API with
caution and only when the more simple overloads cannot be used instead.
