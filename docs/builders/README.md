...
| --- | --- | --- | --- |
| [`services.AddOpenTelemetry()`](./add-opentelemetry.md) | ASP.NET Core, worker services, hosted apps | Yes | Managed by the host |
| [`OpenTelemetrySdk.Create()`](./opentelemetry-sdk-create.md) | Console apps, background jobs, CLI tools | Yes | Single `Dispose()` call |
| [`Sdk.CreateTracerProviderBuilder()`](./sdk-create-tracer-provider-builder.md) | Legacy / <= 1.9.0 codebases, single-signal tests | No (per-signal) | Dispose each provider individually |
