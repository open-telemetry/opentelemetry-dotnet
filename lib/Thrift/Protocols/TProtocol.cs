...
namespace Thrift.Protocols
{
    ...
    public abstract class TProtocol : IDisposable
    {
        ...
        public virtual async Task WriteBinaryAsync(byte[] bytes, CancellationToken cancellationToken)
        {
            await WriteBinaryAsync(bytes, 0, bytes.Length, cancellationToken).ConfigureAwait(false);
        }

        public virtual async Task WriteBinaryAsync(byte[] buffer, int offset, int count)
        {
            await WriteBinaryAsync(buffer, offset, count, CancellationToken.None).ConfigureAwait(false);
        }

        public abstract Task WriteBinaryAsync(byte[] buffer, int offset, int count, CancellationToken cancellationToken);
