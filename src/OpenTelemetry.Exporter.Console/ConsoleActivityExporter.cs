        ...
        foreach (var activity in batch)
        {
            var output = new
            {
                Activity = new
                {
                    ...
                    ParentSpanId = activity.ParentSpanId == default ? default : activity.ParentSpanId,
                    ActivitySourceName = activity.Source.Name,
                    ActivitySourceVersion = string.IsNullOrEmpty(activity.Source.Version) ? null : activity.Source.Version,
                    ...
                    StatusDescription = !string.IsNullOrEmpty(activity.StatusDescription) ? activity.StatusDescription : null,
                    Events = activity.Events.Select(e => new { Name = e.Name, Timestamp = e.Timestamp, Attributes = e.EnumerateTagObjects().Select(a => new { Key = a.Key, Value = a.Value }).ToList() }).ToList(),
                    Links = activity.Links.Select(l => new { TraceId = l.Context.TraceId, SpanId = l.Context.SpanId, Attributes = l.EnumerateTagObjects().Select(a => new { Key = a.Key, Value = a.Value }).ToList() }).ToList(),
                    Resource = this.ParentProvider.GetResource() != Resource.Empty ? this.ParentProvider.GetResource().Attributes.Select(a => new { Key = a.Key, Value = a.Value }).ToList() : null
                },
            };
