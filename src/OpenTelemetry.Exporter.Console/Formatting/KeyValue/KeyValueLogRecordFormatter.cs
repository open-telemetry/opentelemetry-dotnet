...
namespace OpenTelemetry.Exporter.Formatting.KeyValue;

internal sealed class KeyValueLogRecordFormatter : KeyValueFormatterBase<LogRecord>
{
    ...
    public KeyValueLogRecordFormatter(ConsoleExporterOptions options)
        : base(options)
    {
    }
    ...
    public override ExportResult Export(in Batch<LogRecord> batch, ConsoleFormatterContext context)
    {
        ...
        foreach (var logRecord in batch)
        {
            ...
            void ProcessScope(LogRecordScope scope, KeyValueLogRecordFormatter formatter)
            {
                if (++scopeDepth == 0)
                {
