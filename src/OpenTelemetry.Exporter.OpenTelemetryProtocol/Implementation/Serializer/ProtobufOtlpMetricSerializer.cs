            ...
            case MetricType.Histogram:
                {
                    ...
                    foreach (ref readonly var metricPoint in metric.GetMetricPoints())
                    {
                        ...
                        foreach (var histogramMeasurement in metricPoint.GetHistogramBuckets())
                        {
                            ...
                            writePosition += ProtobufSerializer.WriteFixed64WithTag(buffer.AsSpan(writePosition), ProtobufOtlpMetricFieldNumberConstants.HistogramDataPoint_Bucket_Counts, bucketCount);

                            if (histogramMeasurement.ExplicitBound != double.PositiveInfinity)
                            {
                                writePosition += ProtobufSerializer.WriteDoubleWithTag(buffer.AsSpan(writePosition), ProtobufOtlpMetricFieldNumberConstants.HistogramDataPoint_Explicit_Bounds, histogramMeasurement.ExplicitBound);
                            }
                        }
        ...
        if (metricPoint.TryGetExemplars(out var exemplars))
        {
            foreach (ref readonly var exemplar in exemplars)
            {
                writePosition = WriteExemplar(
                    ...
                    static (byte[] buffer, int writePosition, in Exemplar exemplar) => ProtobufSerializer.WriteFixed64WithTag(buffer.AsSpan(writePosition), ProtobufOtlpMetricFieldNumberConstants.Exemplar_Value_As_Int, (ulong)exemplar.LongValue));
            }
        }
        ...
        if (metricPoint.TryGetExemplars(out var exemplars))
        {
            foreach (ref readonly var exemplar in exemplars)
            {
                writePosition = WriteExemplar(
                    ...
                    in exemplar,
                    fieldNumber,
                    static (byte[] buffer, int writePosition, in Exemplar exemplar) => ProtobufSerializer.WriteDoubleWithTag(buffer.AsSpan(writePosition), ProtobufOtlpMetricFieldNumberConstants.Exemplar_Value_As_Double, exemplar.DoubleValue));
            }
        }
