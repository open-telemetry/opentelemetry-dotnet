...
internal static class ProtobufSerializer
{
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    internal static int WriteByteArrayWithTag(Span<byte> buffer, int writePosition, int fieldNumber, ReadOnlySpan<byte> value)
    {
        writePosition = WriteTag(buffer.Slice(writePosition), fieldNumber, ProtobufWireType.LEN);
        writePosition = WriteLength(buffer.Slice(writePosition), value.Length);
        value.CopyTo(buffer.Slice(writePosition));
