...
* `TracesEndpoint` (read only): Target to which the exporter is going to send traces.
  Initially this property is derived from OTEL_EXPORTER_OTLP_ENDPOINT environment variable 
  and adjusted according to the specification. May be overridden by Endpoint property.
* `MetricsEndpoint` (read only): Target to which the exporter is going to send metrics.
  Initially this property is derived from OTEL_EXPORTER_OTLP_ENDPOINT environment variable 
  and adjusted according to the specification. May be overridden by Endpoint property.
