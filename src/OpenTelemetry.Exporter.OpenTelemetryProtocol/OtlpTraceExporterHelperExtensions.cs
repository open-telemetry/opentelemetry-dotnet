...
public static class OtlpTraceExporterHelperExtensions
{
    ...
    /// Adds OpenTelemetry Protocol (OTLP) exporter to the TracerProvider.
    /// </summary>
    /// <param name="builder"><see cref="TracerProviderBuilder"/> builder to
    /// use.</param>
    /// <returns>The instance of <see cref="TracerProviderBuilder"/> to chain
    /// the calls.</returns>
    internal static TracerProviderBuilder AddOtlpTraceExporter(
        this TracerProviderBuilder builder)
        => AddOtlpTraceExporter(builder, name: null, configuration: null, configure: null);
    ...
    /// cref="OtlpTraceExportProcessorBuilder"/>.</param>
    /// <returns>The instance of <see cref="TracerProviderBuilder"/> to chain
    /// the calls.</returns>
    internal static TracerProviderBuilder AddOtlpTraceExporter(
        this TracerProviderBuilder builder,
        Action<OtlpTraceExportProcessorBuilder> configure)
    {
        ...
        return AddOtlpTraceExporter(builder, name: null, configuration: null, configure);
    }
    ...
    /// cref="OtlpTraceExporterOptions"/> and <see
        ...
        IConfiguration configuration)
    {
        ...
        return AddOtlpTraceExporter(builder, name: null, configuration, configure: null);
    }
        ...
        Action<OtlpTraceExportProcessorBuilder>? configure)
    {
        ...
        builder.ConfigureServices(services =>
        {
            services.RegisterOptionsFactory(
                configuration => new OtlpTraceExporterOptions(configuration));

            services.RegisterOptionsFactory(
                (sp, configuration, name) => new ExportActivityProcessorOptions(
                    configuration,
                    sp.GetRequiredService<IOptionsMonitor<BatchExportActivityProcessorOptions>>().Get(name)));

            services.RegisterOptionsFactory(
                configuration => new SdkLimitOptions(configuration));
        });
