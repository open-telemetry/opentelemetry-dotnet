            ...
            lock (this.instrumentCreationLock)
            {
                for (int i = 0; i < maxCountMetricsToBeCreated; i++)
                {
                    ...
                    else
                    {
                        ...
                        double[] histogramBucketBounds = (metricStreamConfig as ExplicitBucketHistogramConfiguration)?.Boundaries;
                        Metric metric = new(instrumentIdentity, this.Temporality, this.maxMetricPointsPerMetricStream, histogramBucketBounds, tagKeysInteresting);
