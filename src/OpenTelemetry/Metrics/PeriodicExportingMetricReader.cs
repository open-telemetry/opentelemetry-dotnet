    ...
    public class PeriodicExportingMetricReader : BaseExportingMetricReader
    {
        ...
        private void ExporterProc()
        {
            ...
            while (true)
            {
                ...
                {
                    case 0: // export
                        OpenTelemetrySdkEventSource.Log.MetricReaderEvent($"{nameof(PeriodicExportingMetricReader)} calling {nameof(MetricReader)}.{nameof(this.Collect)} because Export was triggered.");
                        this.Collect(this.exportTimeoutMilliseconds);
                        break;
                    case 1: // shutdown
                        OpenTelemetrySdkEventSource.Log.MetricReaderEvent($"{nameof(PeriodicExportingMetricReader)} calling {nameof(MetricReader)}.{nameof(this.Collect)} because Shutdown was triggered.");
                    ...
                    case WaitHandle.WaitTimeout: // timer
                        OpenTelemetrySdkEventSource.Log.MetricReaderEvent($"{nameof(PeriodicExportingMetricReader)} calling {nameof(MetricReader)}.{nameof(this.Collect)} because the export interval of {this.exportIntervalMilliseconds} milliseconds has elapsed.");
                        this.Collect(this.exportTimeoutMilliseconds);
                        break;
                }
            }
