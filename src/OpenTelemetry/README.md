...
In addition to logging internal events, Sdk also emits the following metric data
for number of selected internal events:

// TODO (Add list of metrics here)

You can collect these metrics by adding `OpenTelemetry.Sdk` meter name to your
MeterProvider Configuration as shown below:

```csharp
Sdk.CreateMeterProviderBuilder()
   .AddMeter("OpenTelemetry.Sdk")
   .Build();
```
