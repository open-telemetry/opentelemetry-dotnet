...
internal sealed class HttpHandlerMetricsDiagnosticListener(
    string name,
    RequestMethodHelper requestMethodHelper) : ListenerHandler(name)
{
    ...
    private static void OnExceptionEventWritten(Activity activity, object payload)
    {
    ...
    private void OnStopEventWritten(Activity activity, object payload)
    {
        if (TryFetchRequest(payload, out HttpRequestMessage request))
        {
            ...
            tags.Add(new KeyValuePair<string, object>(
                ...
                this.requestMethodHelper.GetNormalizedHttpMethod(request.Method.Method)));
