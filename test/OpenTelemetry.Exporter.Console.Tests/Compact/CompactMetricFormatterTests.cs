...
public class CompactMetricFormatterTests
{
    ...
    public async Task CumulativeMeter()
    {
        ...
        var values0 = lines[0].Split(' ');
        Assert.Contains("1s", values0[3], StringComparison.InvariantCulture);
        Assert.Contains("sum=200", values0[6], StringComparison.InvariantCulture);

        var values1 = lines[1].Split(' ');
        Assert.Contains("2s", values1[3], StringComparison.InvariantCulture);
        Assert.Contains("sum=400", values1[6], StringComparison.InvariantCulture);
    }
