...
namespace OpenTelemetry.Instrumentation.AspNetCore.Tests
{
        ...
        : IClassFixture<WebApplicationFactory<Startup>>, IDisposable
    {
        ...
        public async Task SuccessfulTemplateControllerCallUsesParentContext()
        {
            ...
            Assert.Equal("/api/values/2", activity.TagObjects.FirstOrDefault(i => i.Key == SpanAttributeConstants.HttpPathKey).Value as string);
        ...
        public async Task RequestNotCollectedWhenFilterIsApplied()
        {
            ...
            Assert.Equal(ActivityKind.Server, activity.Kind);
            Assert.Equal("/api/values", activity.TagObjects.FirstOrDefault(i => i.Key == SpanAttributeConstants.HttpPathKey).Value as string);
        }
        ...
        private static void ValidateAspNetCoreActivity(Activity activityToValidate, string expectedHttpPath, Resources.Resource expectedResource)
        {
            ...
            Assert.Equal(expectedHttpPath, activityToValidate.TagObjects.FirstOrDefault(i => i.Key == SpanAttributeConstants.HttpPathKey).Value as string);
