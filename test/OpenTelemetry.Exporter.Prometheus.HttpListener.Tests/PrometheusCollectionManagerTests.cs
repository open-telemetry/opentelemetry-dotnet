...
#pragma warning disable SYSLIB1045 // Convert to 'GeneratedRegexAttribute'.
            Assert.Single(Regex.Matches(output, "^# TYPE otel_scope info$", RegexOptions.Multiline).Cast<Match>());
            Assert.Single(Regex.Matches(output, "^# HELP otel_scope Scope metadata$", RegexOptions.Multiline).Cast<Match>());
#pragma warning restore SYSLIB1045 // Convert to 'GeneratedRegexAttribute'.

            var scopeTypeIndex = output.IndexOf("# TYPE otel_scope info", StringComparison.Ordinal);
            var testMetricTypeIndex = output.IndexOf("# TYPE test_metric gauge", StringComparison.Ordinal);

            Assert.True(scopeTypeIndex >= 0, "No TYPE found for otel_scope.");
            Assert.True(testMetricTypeIndex > scopeTypeIndex, "otel_scope block should appear before test_metric.");
