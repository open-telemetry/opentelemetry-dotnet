...
namespace OpenTelemetry.Metrics.Tests
{
    public class MetricApiTest
    {
        ...
        [Fact]
        public void VerifyObservableCounter_SingleValue()
        {
            ...
            Assert.Equal(2, i);
            Assert.Equal(20, GetLongSum(metrics));
        }

        [Fact]
        public void VerifyObservableCounter_SingleMeasurement()
        {
            var metrics = new List<Metric>();
        ...
        [Fact]
        public void VerifyObservableCounter_MultipleMeasurements()
        {
            var metrics = new List<Metric>();
            ...
            Assert.Single(metrics); // verify that List<metrics> contains 1 item
            Assert.Equal(1, i); // verify that the callback is invoked everytime we call Flush.
            Assert.Equal(100, GetLongSum(metrics)); // only the last value in the list is kept.

            metrics.Clear();
            provider.ForceFlush(MaxTimeToAllowForFlush);
            Assert.Single(metrics);
            Assert.Equal(2, i);
            Assert.Equal(200, GetLongSum(metrics));
        }
