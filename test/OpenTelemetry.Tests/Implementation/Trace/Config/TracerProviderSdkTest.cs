...
{
    public class TracerProviderSdkTest
    {
        [Fact]
        ...
        {
            var tracerProvider = TracerProviderSdk.Create(b => { });
        ...
        public void CreateFactory_BuilderWithArgs()
        {
            ...
            var tracerProvider = TracerProviderSdk.Create(b => b
        ...
        {
            var tracerProvider = TracerProviderSdk.Create(b => { b.SetResource(new Resource(new Dictionary<string, object>() { { "a", "b" } })); });
            var tracer = (TracerSdk)tracerProvider.GetTracer("foo", "semver:1.2.3");
