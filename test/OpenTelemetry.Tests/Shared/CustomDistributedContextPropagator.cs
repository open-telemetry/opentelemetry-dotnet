// <copyright file="CustomDistributedContextPropagator.cs" company="OpenTelemetry Authors">
...
{
    public class CustomDistributedContextPropagator : DistributedContextPropagator
    {
        ...
        private bool customPropagation;
        private ActivityTraceId customActivityTraceId;
        private ActivitySpanId customActivitySpanId;
        private string traceState;

        public CustomDistributedContextPropagator(bool isCustomPropagator, ActivityTraceId traceId = default, ActivitySpanId spanId = default, string tracestate = null)
        {
            this.customPropagation = isCustomPropagator;
            this.customActivityTraceId = traceId;
            this.customActivitySpanId = spanId;
        ...
        public override void ExtractTraceIdAndState(object? carrier, PropagatorGetterCallback? getter, out string? traceId, out string? traceState)
        {
            ...
            if (this.customPropagation)
            {
                traceId = $"00-{this.customActivityTraceId}-{this.customActivitySpanId}-01";
        ...
        public override void Inject(Activity? activity, object? carrier, PropagatorSetterCallback? setter)
        {
            ...
            if (this.customPropagation)
            {
                setter(carrier, TraceParent, $"00-{this.customActivityTraceId}-{this.customActivitySpanId}-01");
                if (!string.IsNullOrEmpty(this.traceState))
                {
                    setter(carrier, TraceState, this.traceState);
                }
            }
            else
            {
