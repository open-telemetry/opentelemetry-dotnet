// <copyright file="Otlp070.cs" company="OpenTelemetry Authors">
...
using Opentelemetry070.Proto.Collector.Metrics.V1;
using Opentelemetry070.Proto.Common.V1;
using Opentelemetry070.Proto.Metrics.V1;
using Opentelemetry070.Proto.Resource.V1;

namespace ProtoBench
{
    public class Otlp070
    {
            ...
            int numQV)
        {
            ...
            for (int lib = 0; lib < numLibs; lib++)
            {
                ...
                for (int m = 0; m < numMetrics; m++)
                {
                    Metric metric = new Metric();
                    metric.Name = $"Metric_{m}";
                    metric.DoubleSummary = new DoubleSummary();

                    for (int tsx = 0; tsx < numTimeseries; tsx++)
                    {
                        var datapoint = new DoubleSummaryDataPoint();
                        ...
                        {
                            var qvalues = new DoubleSummaryDataPoint.Types.ValueAtQuantile();
                        ...
                        metric.DoubleSummary.DataPoints.Add(datapoint);
                    }
                ...
                foreach (var meter in resmetric.InstrumentationLibraryMetrics)
                {
                    ...
                    foreach (var metric in meter.Metrics)
                    {
                        ...
                        switch (metric.DataCase)
                        {
                            ...
                            case Metric.DataOneofCase.DoubleSummary:
                                sumdps = metric.DoubleSummary.DataPoints;
                                break;
